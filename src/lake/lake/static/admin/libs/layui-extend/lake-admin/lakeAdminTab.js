/** tab.js */
layui.define(['element', 'msg'], function (exports) {
    "use strict";
    ...
    Tab.prototype.init = function () {
        ...
        if (typeof (_config.elem) !== 'string' && typeof (_config.elem) !== 'object') {
            msg.throwError('Tab error: elem参数未定义或设置出错，具体设置格式请参考文档API.');
        }
        var $container;
        ...
        if ($container.length === 0) {
            msg.throwError('Tab error:找不到elem参数配置的容器，请检查.');
        }
        var filter = $container.attr('lay-filter');
        if (filter === undefined || filter === '') {
            msg.throwError('Tab error:请为elem容器设置一个lay-filter过滤器');
        }
    ...
    var tab = new Tab();
    exports('lakeAdminTab', function (options) {
